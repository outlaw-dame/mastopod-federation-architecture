...
describe("FollowersSyncActivityPodsClient authority reads", () => {
  it("preserves valid empty state and sends the precise server base URI", async () => {
    ...
    expect(requestedUrl.pathname).toBe("/api/internal/followers-sync-v2/partial-collection");
    expect(requestedUrl.searchParams.get("actorIdentifier")).toBe("alice");
    expect(requestedUrl.searchParams.get("baseUri")).toBe("https://remote.example:8443/");
    expect(requestedUrl.searchParams.has("domain")).toBe(false);
  });

  it("does not collapse an unavailable endpoint into authoritative empty state", async () => {
    const body = responseBody(["unused"]);
  ...
  it("rejects invalid server base input before calling ActivityPods", async () => {
    await expect(client().getPartialFollowers("alice", "not-a-url"))
      .rejects.toMatchObject({ code: "invalid_server_base_uri" });
    expect(requestMock).not.toHaveBeenCalled();
  });

  it("rejects oversized responses before JSON materialization", async () => {
    const body = responseBody([Buffer.alloc(40_000, 0x20), Buffer.alloc(30_000, 0x20)]);
  ...
  it("rejects malformed follower entries instead of filtering them", async () => {
    requestMock.mockResolvedValueOnce({
      statusCode: 200,
