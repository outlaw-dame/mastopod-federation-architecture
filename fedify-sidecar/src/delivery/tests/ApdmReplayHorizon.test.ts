...
describe("APDM replay horizon", () => {
  it("keeps producer replay plus both queue stages inside completed-marker retention", () => {
    ...
    expect(APDM_MAX_AUTOMATIC_DUPLICATE_AGE_MS).toBe(6 * 24 * 60 * 60 * 1000);
    expect(APDM_COMPLETED_DELIVERY_MIN_RETENTION_MS).toBe(7 * 24 * 60 * 60 * 1000);
  ...
  it("rejects non-Redis message ids instead of inventing an enqueue time", () => {
    expect(redisStreamMessageTimestampMs("message-1")).toBeNull();
    expect(redisStreamMessageTimestampMs("0-1")).toBeNull();
    expect(outboundMessageResidenceMs("message-1", 1_000)).toBeNull();
  });
});
