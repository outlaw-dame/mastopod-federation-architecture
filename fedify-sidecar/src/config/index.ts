...
const configSchema = z.object({
  ...
  search: z.object({
    backend: z.enum(['opensearch', 'qdrant', 'dual']).default('opensearch'),
    qdrantUrl: z.string().url().default('http://localhost:6333'),
    qdrantCollectionName: z.string().default('public-content-v1'),
    qdrantVectorSize: z.number().int().positive().default(1024),
  }),
  ...
  delivery: z.object({
    ...
    signatureCacheTtlMs: z.number().default(300000),
...
function loadConfig(): Config {
  const rawBackend = process.env['SEARCH_BACKEND'];
  const searchBackend =
    rawBackend === 'qdrant' || rawBackend === 'dual' || rawBackend === 'opensearch'
      ? rawBackend
      : 'opensearch';
  ...
  const rawConfig = {
    ...
    activitypods: {
      ...
      signingApiUrl:
        process.env['ACTIVITYPODS_SIGNING_API_URL'] ?? 'http://localhost:3000/api/signing',
