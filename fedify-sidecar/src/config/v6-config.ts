...
export const activityPodsConfig = {
  ...
  signingApiUrl:
    process.env['ACTIVITYPODS_SIGNING_API_URL'] ||
    `${process.env['ACTIVITYPODS_URL'] || 'http://localhost:3000'}/api/internal/signatures/batch`,
  inboxReceiverUrl:
    process.env['ACTIVITYPODS_INBOX_RECEIVER_URL'] ||
...
export function validateConfig(): string[] {
  const errors: string[] = [];
  if (!activityPodsConfig.url) errors.push('ACTIVITYPODS_URL is required');
  if (!activityPodsConfig.token) errors.push('ACTIVITYPODS_TOKEN is required');
  if (!redisConfig.url) errors.push('REDIS_URL is required');
  if (redpandaConfig.brokers.length === 0) errors.push('REDPANDA_BROKERS is required');
  return errors;
}
...
export function logConfiguration(): void {
  ...
  console.log(`Search backend: ${featureFlags.searchBackend}`);
