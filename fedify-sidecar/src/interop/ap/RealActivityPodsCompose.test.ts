...
describe("real ActivityPods sidecar compose authority", () => {
  ...
  it("keeps SSRF-protected target tunnels path-restricted and readiness-gated", () => {
    ...
    expect(friendicaBootstrap).toContain("ensure_config config logfile /var/log/friendica/friendica.log");
    expect(friendicaBootstrap).toContain("ensure_config config loglevel info");
