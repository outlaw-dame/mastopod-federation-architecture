...
describe("real ActivityPods sidecar compose authority", () => {
  ...
  it("keeps SSRF-protected target tunnels path-restricted and readiness-gated", () => {
    ...
    expect(workflow).toContain('authorityContinuity":"same-bounded-tunnel-and-proxy"');
    ...
    expect(workflow).toContain('mv "${route_file}.next" "${route_file}"');
    expect(workflow.indexOf('[[ "${ready}" == true ]]')).toBeLessThan(workflow.indexOf('printf \'%s\\n\' external'));
    expect(workflow).not.toContain("external-public-proxy.pid");
