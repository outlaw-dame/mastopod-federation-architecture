...
export interface StoredExternalAtSession {
  ...
  /**
   * JSON-serialized ES256 DPoP private JWK bound to this session.
   * Present when the account was linked via ATProto OAuth (not password).
   * The access/refresh tokens are DPoP-bound — every XRPC request MUST
   * include a signed DPoP proof using this key and the Authorization header
   * MUST use `DPoP <token>` (not `Bearer <token>`).
   * Generated by OAuthDpopKeyManager.generateDpopKeypair().
   */
  dpopPrivateKeyJwk?: string;
  /**
   * OAuth token endpoint URL for this PDS (from authorization-server discovery).
   * Present when dpopPrivateKeyJwk is set; used for DPoP-bound token refresh
   * via the OAuth token endpoint instead of the legacy XRPC refreshSession path.
   */
  tokenEndpoint?: string;
  /**
   * Authorization server issuer for this session (from discovery metadata).
   * Stored for auditing and for constructing correct DPoP `htu` values.
   */
  oauthIssuer?: string;
}
