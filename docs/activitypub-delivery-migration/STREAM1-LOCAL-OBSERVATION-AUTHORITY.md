...
A lifecycle record and the ActivityPub lifecycle activity are separate outputs. A federation-public local `Delete`, or a relevant `Undo` identified by ActivityPods' lifecycle metadata, remains an ActivityPub activity and therefore belongs to Stream1 and `ap.firehose.v1`. The corresponding deletion signal is additionally written to `ap.tombstones.v1`.

This is symmetric with the remote path: public remote lifecycle activity belongs to Stream2/firehose while its tombstone remains on the separate lifecycle topic. An unrelated `Undo` remains a normal public ActivityPub event and must not acquire tombstone semantics merely because its type is `Undo`.
...
- the same committed `createdAt` value as the Stream1 publication timestamp and tombstone timestamp when applicable.
...
| Activity | Stream1 | Tombstone | Remote fan-out |
| --- | --- | --- | --- |
| local public, zero remote recipients | yes | no | none |
| local public, remote recipients | yes | no | yes |
| local unlisted, zero or more remote recipients | yes | no | only when targets exist |
| local public Delete / relevant Undo | yes | yes | only when targets exist |
| unrelated local public Undo | yes | no | only when targets exist |
| local followers-only | no | lifecycle-only when applicable | according to authoritative recipients |
| local direct | no | lifecycle-only when applicable | according to authoritative recipients |
| remote public | no; Stream2 | lifecycle-only when applicable | inbound path |
| tombstone record itself | no | dedicated tombstone topic | separate lifecycle handling |
