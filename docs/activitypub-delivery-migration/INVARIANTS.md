...
10. Any defer, retry, or DLQ transition that replaces a consumed Redis Stream message must durably persist the replacement/recovery record **before** acknowledging the source message. Future-dated replacements are written directly to the non-trimmed delayed store instead of being exposed first to the `MAXLEN` ready Stream. If a replacement, parking, or DLQ write fails, the original source remains pending and reclaimable.
11. The eight-day completed-marker invariant applies across upgrades, not only to markers created by new code. The legacy unversioned namespace is upgraded through a one-time maintenance cutover: deploy the bounded ActivityPods producer first, stop all legacy outbound sidecar workers, start the upgraded sidecar with `APDM_COMPLETION_MARKER_V2_CUTOVER=maintenance` when legacy markers exist, atomically convert legacy markers to `ap:delivery:completed:v2:*` before queue consumption, write a permanent migration sentinel, and then run only upgraded workers. Fresh Redis installations with no legacy markers write the sentinel automatically. There is no periodic TTL-extension sweep and no supported mixed-version marker-writing window after the sentinel is established.
