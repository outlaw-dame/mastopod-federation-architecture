...
OpenSearch mappings are additive. An existing `public-content-v1` index created by an older release can retain historical vector fields after an in-place upgrade because field removal is not a valid mapping update.

That does **not** make those fields active OS3 requirements:

- OS3 does not create the embedding ingest pipeline;
...
If that contract fails against 3.8, the client upgrade becomes part of OS3 and must be validated before merge. If it passes, a client-major upgrade can be handled separately from the server migration rather than combining two breaking-change surfaces without evidence.
